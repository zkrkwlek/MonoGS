...
def simple_nms(scores: Tensor, nms_radius: int):
    ...
    for _ in range(2):
        ...
        max_mask.float(), kernel_size=nms_radius * 2 + 1, stride=1, padding=nms_radius) > 0
...
class DKD(nn.Module):
    def __init__(self, radius: int=2, top_k: int=0, scores_th: float=0.2, n_limit:int=20000, script: bool=False):
        """
        Args:
